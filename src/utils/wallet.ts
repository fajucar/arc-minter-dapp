...
export async function connectWallet(preferredWalletName?: string): Promise<WalletState> {
  ...
  try {
    ...
    // Create provider AFTER account request
    let provider = new BrowserProvider(providerEthereum as any);
    let network = await provider.getNetwork();
    let chainId = Number(network.chainId);
    
    // Ensure we're on Arc Testnet or Localhost
    if (chainId !== ARC_TESTNET.chainId && chainId !== LOCALHOST_NETWORK.chainId) {
      ...
      // IMPORTANT: after chain switch, re-create provider and re-read network
      provider = new BrowserProvider(providerEthereum as any);
