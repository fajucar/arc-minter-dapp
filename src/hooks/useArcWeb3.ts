...
// Import ABIs
// GiftCardMinter ABI - using inline definition instead of JSON import for build compatibility
const GiftCardMinterABI = [
  {
    name: 'mintImageNFT',
    type: 'function',
...
export function useArcWeb3() {
  ...
  const mintNFT = async (id: number | string) => {
    ...
    try {
      ...
      // GiftCardMinterABI is already the ABI array, no need for .abi
      const minterContract = new ethers.Contract(
        CONTRACT_ADDRESSES.GIFT_CARD_MINTER,
        GiftCardMinterABI as any, // Type assertion needed for ethers.Contract
        signer
      );
      ...
      // Find the mint event to get the token ID
      const mintEvent = receipt.logs.find((log: any) => {
        try {
          const parsed = minterContract.interface.parseLog(log);
          return parsed && parsed.name === 'ImageNFTRequested';
        } catch {
          return false;
        }
      });
      ...
      if (mintEvent) {
        const parsed = minterContract.interface.parseLog(mintEvent);
        console.log('NFT minted with token ID:', parsed?.args.tokenId.toString());
      }
