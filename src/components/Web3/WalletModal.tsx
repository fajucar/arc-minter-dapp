...
export function WalletModal({ isOpen, onClose }: WalletModalProps) {
  ...
  const wallets = useMemo(() => {
    ...
    // Sempre mostra todas as opções, mesmo que não instaladas
    const list = [
    ...
    // Log para debug
    console.log('[WalletModal] Detected wallets:', {
      hasMetaMask,
      hasRabby,
      hasCoinbase,
      hasOkx,
      providers: getInjectedProviders().length,
    })
  ...
  const handleConnect = async (_walletId: string) => {
    try {
      // For all wallets, use injected connector (browser will show wallet selection)
