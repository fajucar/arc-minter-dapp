...
export function ConfigError() {
  ...
  if (import.meta.env.DEV) {
    console.log('[ConfigError] Checking CONTRACT_ADDRESSES.GIFT_CARD_NFT:', CONTRACT_ADDRESSES.GIFT_CARD_NFT);
    console.log('[ConfigError] Value is empty?', !CONTRACT_ADDRESSES.GIFT_CARD_NFT || CONTRACT_ADDRESSES.GIFT_CARD_NFT === '');
  }

  const missingAddresses = [];
  
  // NFT address is required - check if BOTH variables are missing
  // VITE_GIFT_CARD_NFT_ADDRESS is primary, VITE_ARC_COLLECTION_ADDRESS is fallback
  // Error should ONLY appear if BOTH are missing or empty
  if (!CONTRACT_ADDRESSES.GIFT_CARD_NFT || CONTRACT_ADDRESSES.GIFT_CARD_NFT === '') {
    missingAddresses.push('VITE_GIFT_CARD_NFT_ADDRESS or VITE_ARC_COLLECTION_ADDRESS');
  }
  
  // MOCK_USDC and MINTER are optional - don't add to missingAddresses
  // They will be handled gracefully in the code
            ...
            <ol className="list-decimal list-inside mt-2 space-y-2">
              <li>
                ...
                <ul className="list-disc list-inside ml-4 mt-1 space-y-1">
                  ...
                  <li>Crie o arquivo <code className="bg-yellow-100 px-1 rounded">.env</code> na raiz do projeto com os endereços:</li>
                </ul>
                <pre className="mt-2 bg-yellow-100 p-2 rounded text-xs overflow-x-auto">
{`VITE_GIFT_CARD_NFT_ADDRESS=0x...
VITE_ARC_COLLECTION_ADDRESS=0x... (opcional, fallback)
VITE_GIFT_CARD_MINTER_ADDRESS=0x... (opcional)
VITE_MOCK_USDC_ADDRESS=0x... (opcional)`}
                </pre>
              </li>
              <li>
